...
class EvaluationFramework:
        ...
        with torch.no_grad():
            for sequence in sequences:
                ...
                if 'beta_vae' in model_name.lower():
                    ...
                    loss, recon, mean, logvar = model(input_tensor.unsqueeze(0))
                    recon_error = torch.mean((input_tensor - recon.squeeze(0)) ** 2).item()
                    
                elif 'world_model' in model_name.lower():
                    # World Model evaluation
                    obs_sequence = torch.stack(tensor_sequence[:-1]).unsqueeze(0)
                    action_sequence = torch.zeros(1, len(tensor_sequence)-1, 10, device=self.device)
                    actions, hidden, encoded = model(obs_sequence, action_sequence)
                    # Use encoding quality as proxy for reconstruction
                    recon_error = torch.mean(encoded ** 2).item()
                    
                elif 'transformer' in model_name.lower():
                    # Transformer evaluation
                    predictions = model(tensor_sequence[:-1])
                    target = tensor_sequence[-1]
                    recon_error = torch.mean((predictions[0, -1] - target) ** 2).item()
                    
                elif 'ode' in model_name.lower():
                    # Neural ODE-VAE evaluation
                    recon, mean, logvar, z_t = model(tensor_sequence)
                    recon_error = torch.mean((tensor_sequence[-1] - recon) ** 2).item()
                
                else:
                    # Generic evaluation
                    recon_error = 0.1  # Placeholder
    ...
    def _analyze_results(self, results: Dict[str, Dict[str, float]]) -> Dict[str, Any]:
        ...
        for metric in metrics:
            metric_scores = {model: scores[metric] for model, scores in results.items()}
    ...
    def generate_test_report(self, analysis, output_path='benchmark_report.md'):
        ...
        with open(output_path, 'w') as f:
            ...
            for metric, ranking in analysis['rankings'].items():
                ...
                best_baseline_score = ranking[0][1] if ranking[0][0] != 'neurocious' else ranking[1][1]
...
async def run_example_benchmark():
    ...
    from neurocious_core import CoTrainingConfig
