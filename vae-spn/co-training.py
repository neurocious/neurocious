...
from neurocious_core import CoTrainingConfig, reparameterize
from enhanced_vae import EnhancedVAE, FieldAwareKLDivergence
from spatial_probability_network import SpatialProbabilityNetwork
...
class EpistemicCoTraining:
    ...
    async def train_step(
        ...
        # Move data to device
        input_sequence = [x.to(self.device) for x in input_sequence]
        expected_actions = [x.to(self.device) for x in expected_actions]
        observed_reactions = [x.to(self.device) for x in observed_reactions]
        future_states = [x.to(self.device) for x in future_states]
    ...
    async def _process_timestep(
        ...
        (routing, confidence, policy, reflexes, predictions,
         field_params, explanation, inverse_explanation) = self.spn.process_state(latent_sequence[t])
        ...
        # === VAE Losses ===
        # Reconstruction
        reconstruction, _ = self.vae.decode_with_field(latent_sequence[t])
    ...
    def _calculate_reconstruction_loss(self, input_tensor: torch.Tensor, output: torch.Tensor) -> torch.Tensor:
        ...
        return F.binary_cross_entropy(output, input_tensor)
...
class InverseFlowIntegration:
    ...
    def __init__(
        ...
        from spatial_probability_network import InverseFlowField
    ...
    def reconstruct_prior_belief(
        ...
        from neurocious_core import TemporalRegularizer
