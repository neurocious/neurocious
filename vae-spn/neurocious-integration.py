...
from epistemic_cotraining import EpistemicCoTraining, InverseFlowIntegration
            ...
            if sequences:
                ...
                for seq in sample_sequences:
                    if seq:
                        ...
                        # Reconstruction loss
                        recon_loss = F.binary_cross_entropy(reconstruction, seq_tensors[-1])
                        total_recon_loss += recon_loss.item()
                        
                        # Process through SPN
                        latent = mean  # Use mean for validation
                        (routing, confidence, policy, reflexes, predictions, 
                         field_params, explanation, inverse_explanation) = self.spn.process_state(latent)
