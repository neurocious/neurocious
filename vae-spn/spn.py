...
from neurocious_core import (
    ...
    WorldBranch, normalize_vector_field, reparameterize
)
from enhanced_vae import EnhancedVAE, PolicyNetwork, ReflexNetwork, PredictionNetwork
...
class SpatialProbabilityNetwork(nn.Module, ISpatialProbabilityNetwork):
    ...
    def __init__(
        ...
        # Neural components
        self.policy_network = PolicyNetwork(state_dim)
        self.reflex_network = ReflexNetwork(state_dim)
        self.prediction_network = PredictionNetwork(state_dim)
    ...
    def process_state(self, state: torch.Tensor) -> Tuple[
        ...
        policy, _ = self.policy_network(state, history_tensor)
        
        # Check reflexes
        reflexes = self.reflex_network(state)
    ...
    def _calculate_route_signature(self, state: torch.Tensor) -> str:
        ...
        return ",".join([f"{x:.2f}" for x in rounded_state.cpu().numpy()])
    ...
    def _get_history_tensor(self) -> torch.Tensor:
        ...
        if not self.temporal_buffer:
            return torch.zeros(self.buffer_size * self.state_dim, device=self.device)
        
        # Pad or truncate history to buffer size
        history_list = list(self.temporal_buffer)
        while len(history_list) < self.buffer_size:
            history_list.insert(0, torch.zeros_like(history_list[0]) if history_list else 
                               torch.zeros(self.state_dim, device=self.device))
        
        history_tensor = torch.cat(history_list[-self.buffer_size:])
    ...
    def _generate_belief_explanation(
        ...
        for i in range(self.vector_dim):
            contribution = float(latent_data[i] * np.mean(vector_field_flat[:, i]))
    ...
    def analyze_field_flow(self, state: torch.Tensor, steps: int = 10) -> FlowPattern:
        ...
        for _ in range(steps):
            ...
            routing, _, field_params, _ = self._route_state_internal([current_state])
    ...
    def _calculate_local_curvature(self, state: torch.Tensor, routing: torch.Tensor) -> torch.Tensor:
        ...
        perturbed_routing, _, _, _ = self._route_state_internal([perturbed_state])
    ...
    def _calculate_local_divergence(self, state: torch.Tensor, routing: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(state.shape[0]):
            ...
            forward_routing, _, _, _ = self._route_state_internal([forward_state])
            backward_routing, _, _, _ = self._route_state_internal([backward_state])
    ...
    def _calculate_local_rotation(self, state: torch.Tensor, routing: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(min(state.shape[0] - 1, 3)):  # Limit computation
            for j in range(i + 1, min(state.shape[0], i + 4)):
                ...
                routing_i, _, _, _ = self._route_state_internal([state_i])
                routing_j, _, _, _ = self._route_state_internal([state_j])
    ...
    def _calculate_flow_stability(self, history: List[GeometricField]) -> float:
        ...
        for i in range(1, len(history)):
            ...
            dir_similarity = np.dot(history[i].direction, history[i-1].direction) / (
                np.linalg.norm(history[i].direction) * np.linalg.norm(history[i-1].direction) + 1e-10
            )
...
class InverseFlowField(IInverseFlowField):
    ...
    def _compute_contextual_routing(self, warped_state: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        ...
        attention = F.softmax(torch.matmul(warped_state, encoded_context.T), dim=-1)
        
        # Weight inverse vectors by attention
        field_flat = self.inverse_vector_field.view(-1, self.vector_dim)
        return torch.matmul(attention, field_flat)
    ...
    def _apply_inverse_transformation(
        ...
        return torch.matmul(transformed, field_flat.T)
...
class WarpingModule(nn.Module):
    ...
    def warp_state(self, state: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        ...
        attention_weights = F.softmax(
            torch.matmul(torch.matmul(hidden, self.attention.weight), context.T), 
            dim=-1
        )
        attention_output = torch.matmul(attention_weights, context)
