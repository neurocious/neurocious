...
from baseline_comparisons import BenchmarkSuite
...
class NeurociousTrader:
    ...
    def _market_state_to_features(self, market_state: MarketState) -> torch.Tensor:
        ...
        # Pad to 784 dimensions (to match VAE input size)
        padded_features = np.zeros(784)
    ...
    def _interpret_regime_belief(self, routing: np.ndarray) -> Dict[str, float]:
        ...
        quadrants = {
            'bull': np.sum(routing_2d[:8, :8]),      # Top-left
            'bear': np.sum(routing_2d[8:, :8]),     # Bottom-left  
            'volatile': np.sum(routing_2d[:8, 8:]), # Top-right
            'stable': np.sum(routing_2d[8:, 8:])    # Bottom-right
        }
